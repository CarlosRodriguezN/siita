...
function drawHideCoordenada(coordenadaReg) {
    ...
    if ( oGrafico.lstCoordenadas.length > 0 ) {
        if (oGrafico.idTipoGrafico != 4 ) {
            for (var k = 0; k < oGrafico.lstCoordenadas.length; k++) {
                if (oGrafico.lstCoordenadas[k].regCoordenada = coordenadaReg) {
                    if (!oGrafico.lstCoordenadas[k].gMarker) {
                        setMarkerMap(k);
                        jQuery('#dcg-' + k).html( JSL_COORDENADA_OCULTAR );
                    } else{
                        oGrafico.lstCoordenadas[k].gMarker.setMap(null);
                        oGrafico.lstCoordenadas[k].gMarker = false;
                        jQuery('#dcg-' + k).html( JSL_COORDENADA_VER );
                    }
                }
            }
        } else {
            if (!oGrafico.lstCoordenadas[0].gMarker) {
                setMarkerMap(0);
                jQuery('#dcc-1').html( JSL_COORDENADA_OCULTAR );
            } else{
                oGrafico.lstCoordenadas[0].gMarker.setMap(null);
                oGrafico.lstCoordenadas[0].gMarker = false;
                jQuery('#dcc-1').html( JSL_COORDENADA_VER );
            }
        }
    }
    return grafico;
...
function setMarkerMap(id){
        var glPoint = getGLatlng(parseFloat(oGrafico.lstCoordenadas[id].lat), parseFloat(oGrafico.lstCoordenadas[id].lng));
        //  Creo el Marker
        var pointMarker = new google.maps.Marker({
            position: glPoint,
        });
        oGrafico.lstCoordenadas[id].gMarker = pointMarker;
