 ...
 */
class contratosModelPersonas extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intIdPersona_pc',
                'strApellidos_pc',
                'strNombres_pc',
                'strCedula_pc',
                'strCorreoElectronico_pc',
                'strTelefono_pc',
                'strCelular_pc'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_contratos&amp;task=persona.edit&amp;intIdPersona_pc=' . $item->intIdPersona_pc);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    pers.intIdPersona_pc            AS idPersona,
                            pers.strApellidos_pc            AS apellido,
                            pers.strNombres_pc              AS nombre,
        ...
        $query->from('#__ctr_persona AS pers');
        ...
        if ($published == '') {
            $query->where('( pers.published = 1 OR pers.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("pers.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " pers.intIdPersona_pc LIKE '" . $search . "' OR"
                    . " pers.strApellidos_pc LIKE '" . $search . "' AND"
                    . " pers.strNombres_pc LIKE '" . $search . "' AND"
                    . " pers.strCedula_pc LIKE '" . $search . "' AND"
                    . " pers.strCorreoElectronico_pc LIKE '" . $search . "' AND"
                    . " pers.strTelefono_pc LIKE '" . $search . "' AND"
                    . " pers.strCelular_pc LIKE '" . $search . "'";
