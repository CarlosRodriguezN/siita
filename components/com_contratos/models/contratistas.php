 ...
 */
class contratosModelContratistas extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intIdContratista_cta',
                'intIdTipoContratista_tpocta',
                'strContratista_cta',
                'strRUP',
                'dteFechaRegistro_cta'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_contratos&amp;task=contratista.edit&amp;intIdContratista_cta=' . $item->intIdContratista_cta);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    ctris.intIdContratista_cta          AS idContratista,
                            ctris.intIdTipoContratista_tpocta   AS idTipoContratista,
                            tpCtris.strDescripcion_tpocta       AS tipoContratista,
                            ctris.strContratista_cta            AS Contratista,
                            ctris.strRUP                        AS RUP,
                            ctris.dteFechaRegistro_cta          AS fechaRegistro ,
                            ctris.published');
        
        $query->from('#__ctr_contratista AS ctris');
        
        // tipo de contratista
        $query->leftJoin('#__ctr_tipo_contratista AS tpCtris on tpCtris.intIdTipoContratista_tpocta = ctris.intIdTipoContratista_tpocta');
        ...
        if ($published == '') {
            $query->where('( ctris.published = 1 OR ctris.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("ctris.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " ctris.intIdContratista_cta LIKE '" . $search . "' OR"
                    . " ctris.intIdTipoContratista_tpocta LIKE '" . $search . "' OR"
                    . " ctris.strContratista_cta LIKE '" . $search . "' OR"
                    . " tpCtris.strDescripcion_tpocta LIKE '" . $search . "' OR"
                    . " ctris.strRUP LIKE '" . $search . "' OR"
                    . " ctris.dteFechaRegistro_cta LIKE '" . $search . "'";
