 ...
 */
class contratosModelEstadosGarantia extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intIdEstadoGarantia_eg',
                'intCodEstGarantia_eg',
                'strNombre_eg	',
                'strDescripcion_eg'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_contratos&amp;task=estadogarantia.edit&amp;intIdEstadoGarantia_eg=' . $item->intIdEstadoGarantia_eg);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    etg.intIdEstadoGarantia_eg  AS idEstadoGarantia,
                            etg.intCodEstGarantia_eg    AS codEstadoGarantia,
                            etg.strNombre_eg            AS nombre,
                            etg.strDescripcion_eg       AS descripcion,
                            etg.published');

        $query->from('#__ctr_estado_garantia AS etg');
        ...
        if ($published == '') {
            $query->where('( etg.published = 1 OR etg.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("etg.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " etg.intIdEstadoGarantia_eg LIKE '" . $search . "' OR"
                    . " etg.intCodEstGarantia_eg	 LIKE '" . $search . "' OR"
                    . " etg.strNombre_eg LIKE '" . $search . "' OR"
                    . " etg.strDescripcion_eg LIKE '" . $search . "'";
