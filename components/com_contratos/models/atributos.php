 ...
 */
class contratosModelAtributos extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intIdAtributo_attr',
                'intIdContrato_ctr',
                'intCodAtributo_attr',
                'strNombre_attr',
                'dcmValor_attr',
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_contratos&amp;task=atributo.edit&amp;intIdAtributo_attr=' . $item->intIdAtributo_attr);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    att.intIdAtributo_attr      AS idAtributo,
                            att.intIdContrato_ctr       AS idContrato,
                            ctr.strDescripcion_ctr      AS contrato,
                            att.intCodAtributo_attr     AS codAtributo,
                            att.strNombre_attr          AS nombre,
                            att.dcmValor_attr           AS valor,
                            att.published');

        $query->from('#__ctr_atributo AS att');

        $query->leftJoin('#__ctr_contrato AS ctr ON ctr.intIdContrato_ctr = att.intIdContrato_ctr');
        //
        ...
        if ($published == '') {
            $query->where('( att.published = 1 OR att.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("att.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " att.intIdAtributo_attr LIKE '" . $search . "' OR"
                    . " att.intIdContrato_ctr LIKE '" . $search . "' OR"
                    . " att.intCodAtributo_attr LIKE '" . $search . "' OR"
                    . " att.strNombre_attr LIKE '" . $search . "' OR"
                    . " att.dcmValor_attr LIKE '" . $search . "'";
