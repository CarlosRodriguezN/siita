 ...
 */
class contratosModelMultas extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intIdMulta_mta',
                'intIdContrato_ctr',
                'intCodMulta_mta',
                'dcmMonto_mta'
            );
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_contratos&amp;task=multa.edit&amp;intIdMulta_mta=' . $item->intIdMulta_mta);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    mlt.intIdMulta_mta      AS idMulta,
                            mlt.intIdContrato_ctr   AS idContrato,
                            ctr.intNumContrato_ctr  AS numeroContrato,
                            ctr.strDescripcion_ctr  AS descContrato,
        ...
        $query->leftJoin('#__ctr_contrato AS ctr ON ctr.intIdContrato_ctr = mlt.intIdContrato_ctr');
        //
        ...
        if ($published == '') {
            $query->where('( mlt.published = 1 OR mlt.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("mlt.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " mlt.intIdMulta_mta LIKE '" . $search . "' OR"
                    . " mlt.intIdContrato_ctr LIKE '" . $search . "' OR"
                    . " mlt.intCodMulta_mta LIKE '" . $search . "' OR"
                    . " mlt.dcmMonto_mta LIKE '" . $search . "' OR"
                    . " mlt.strObservacioin_mta LIKE '" . $search . "'";
