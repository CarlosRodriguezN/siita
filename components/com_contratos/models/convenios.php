 ...
 */
class contratosModelConvenios extends JModelList {
    ...
    public function getItems() {
        ...
        foreach( $items as &$item ){
            //  $item->url = JRoute::_('index.php?option=com_contratos&amp;task=convenio.edit&amp;intIdContrato_ctr=' . $item->intIdContrato_ctr);
            $item->url = JRoute::_( 'index.php?option=com_contratos&amp;tpoContrato=2&amp;task=convenio.editConvenio&amp;intIdContrato_ctr=' . $item->intIdContrato_ctr );

            //  $item->url = JRoute::_('index.php?option=com_contratos&amp;tpoContrato=1&amp;task=contrato.edit&amp;intIdContrato_ctr=' . $item->intIdContrato_ctr);
            //  $item->url = JRoute::_('index.php?option=com_contratos&amp;tpoContrato=2&amp;task=contrato.edit&amp;intIdContrato_ctr=' . $item->intIdContrato_ctr);
        }
    ...
    public function getListQuery() {
        ...
        $query = parent::getListQuery();
                            ...
                            //ctr.intIdFiscalizador_fc    AS idFiscalizadorCtr,
                            //cntr.strContratista_cta     AS contratista,
                            //ctr.intIdContratista_cta    AS idContratistaCtr,
        $query->select('    DISTINCT 
                            ctr.intIdContrato_ctr       AS idCtr,
        ...
        if ($published == '') {
            $query->where('( ctr.published = 1 OR ctr.published = 0 )');
        } else if ($published != '*') {
        ...
        if (!empty($search)) {
            ...
            $field_searches = " ctr.strCUR_ctr LIKE '" . $search . "' OR"
                    . " ctr.strDescripcion_ctr LIKE '" . $search . "' AND"
                    . " ctr.strObservacion_ctr LIKE '" . $search . "'";
