 ...
 */
class contratosModelFiscalizadores extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intIdFiscalizador_fc',
                'intIdPersona_pc',
                'strRUC_fc',
                'dteFechaRegistro_fc'
            );
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_contratos&amp;task=fiscalizador.edit&amp;intIdFiscalizador_fc=' . $item->intIdFiscalizador_fc);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    fis.intIdFiscalizador_fc    AS idFiscalizador,
                            fis.intIdPersona_pc         AS idPersona,
                            pers.strApellidos_pc        AS perApellido,
                            pers.strNombres_pc          AS perNombre,
                            fis.strRUC_fc               AS ruc,
                            fis.dteFechaRegistro_fc     AS fchRegistro,
                            fis.published');

        $query->from('#__ctr_fiscalizador AS fis');

        // tipo de contratista
        $query->leftJoin('#__ctr_persona AS pers on pers.intIdPersona_pc = fis.intIdPersona_pc');
        ...
        if ($published == '') {
            $query->where('( fis.published = 1 OR fis.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("fis.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " fis.intIdFiscalizador_fc LIKE '" . $search . "' OR"
                    . " fis.intIdPersona_pc LIKE '" . $search . "' OR"
                    . " fis.strRUC_fc LIKE '" . $search . "' OR" 
                    . " fis.dteFechaRegistro_fc LIKE '" . $search . "'";
