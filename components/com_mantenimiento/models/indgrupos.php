 ...
 */
class MantenimientoModelIndGrupos extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'ig.intId_gpo',
                                                'ig.intId_gpo_padre',
                                                'ig.strDescripcion_gpo',
                                                'ig.published'
                                             );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=indgrupo.edit&amp;intId_gpo=' . $item->intId_gpo);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   ig.intId_gpo,
                           ig.strDescripcion_gpo,
                           ig.intId_gpo_padre,
                           igp.strDescripcion_gpo AS descPadre,
                           ig.published,
                           u.name');
        $query->from('#__ind_grupo as ig');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__ind_grupo igp on igp.intId_gpo = ig.intId_gpo_padre');
        $query->leftJoin('#__users as u on u.id = ig.checked_out');
        ...
        if ($published == '') {
            ...
            $query->where("ig.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " ig.strDescripcion_lbind LIKE '" . $search . "'";
