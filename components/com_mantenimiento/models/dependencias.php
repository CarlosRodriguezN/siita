...
class MantenimientoModelDependencias extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'inpCodigo_depins',
                                                'strDescripcion_depins',
                                                'published' );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=dependencia.edit&amp;inpCodigo_depins='. $item->inpCodigo_depins );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select('    dpen.inpCodigo_depins, 
                            dpen.strDescripcion_depins ,
                            dpen.published,
                            u.name');
        $query->from('#__pei_dependencia as dpen');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = dpen.checked_out');
        ...
        if( $published == '' ){
            $query->where( '( dpen.published = 1 OR dpen.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "dpen.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " dpen.strDescripcion_depins LIKE '". $search ."'";
