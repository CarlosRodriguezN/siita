 ...
 */
class mantenimientoModelSectores extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'inpCodigo_sec', 
                                                'strCodigoCategoria',
                                                'strDescripcion_sec'
                                                 );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=sector.edit&amp;id='. $item->inpCodigo_sec );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   se.inpCodigo_sec,
                            se.strCodigoCategoria,
                            se.strDescripcion_sec,
                            se.published,
                            cat.strDescripcion_categoria,
                            u.name' );
        $query->from( '#__gen_sector se' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__gen_categoria as cat on cat.strCodigoCategoria = se.strCodigoCategoria	' );
        $query->leftJoin( '#__users as u on u.id = se.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( se.published = 1 OR se.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "se.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " se.inpCodigo_sec LIKE '". $search ."' OR"
                            ." se.strCodigoCategoria LIKE '". $search ."' OR"
