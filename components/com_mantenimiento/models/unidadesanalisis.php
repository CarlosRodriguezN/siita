 ...
 */
class mantenimientoModelUnidadesanalisis extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'inpCodigo_unianl',
                                                'strDescripcion_unianl');
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=unidadanalisis.edit&amp;inpCodigo_unianl=' . $item->inpCodigo_unianl);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    uniana.inpCodigo_unianl, 
                            uniana.strDescripcion_unianl ,
                            uniana.published,
                            u.name');
        $query->from('#__gen_unidad_analisis as uniana');

        //  Verificamos si algun registro esta siendo editado por algun usuario

        $query->leftJoin('#__users as u on u.id = uniana.checked_out');
        ...
        if ($published == '') {
            $query->where('( uniana.published = 1 OR uniana.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("uniana.published = '" . $published . "'");
        }
