...
class mantenimientoModelVariables extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intCodigo_var',
                                                'inpCodigo_tipovar',
                                                'strDescripcion_var',
                                                'dteFechaRegistro_var',
                                                'dteFechaModificacion_var',
                                                'strModuloSiita'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=variable.edit&amp;intCodigo_var=' . $item->intCodigo_var);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   vari.intCodigo_var,
                           vari.inpCodigo_tipovar,
                           vari.strDescripcion_var,
                           vari.dteFechaRegistro_var,
                           tipovar.strDescripcion_tipovar,
                           vari.dteFechaModificacion_var,
                           vari.published,
                           u.name');
        $query->from('#__gen_variables as vari');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__gen_tipo_variable as tipovar on tipovar.inpCodigo_tipovar = vari.inpCodigo_tipovar');
        $query->leftJoin('#__users as u on u.id = vari.checked_out');
        ...
        if ($published == '') {
            $query->where('( vari.published = 1 OR vari.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("vari.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " vari.strDescripcion_var LIKE '" . $search . "'";
