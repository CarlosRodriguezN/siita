 ...
 * Modelo tipo obra
 */
class MantenimientoModelAgenda extends JModelAdmin
{
    /**
    ...
    * @since	1.6
    */
    public function getTable( $type = 'Agenda', $prefix = 'MantenimientoTable', $config = array() ) 
    {
    ...
    */
    ...
    {
        // Get the form.
        $form = $this->loadForm( 'com_mantenimiento.agenda', 'agenda', array( 'control' => 'jform', 'load_data' => $loadData ) );
    ...
    */
    protected function loadFormData() 
    {
        ...
        $data = JFactory::getApplication()->getUserState( 'com_mantenimiento.edit.agenda.data', array() );
    ...
    {
        $tbDetalleAgd = $this->getTable( 'DetalleAgd', 'MantenimientoTable' );
    ...
    {
        $tbEstructuraAgd = $this->getTable( 'EstructuraAgd', 'MantenimientoTable' );
    ...
    public function getItemsAgd( $estructuraAgd )
    {
        ...
        //  Obtengo los items padre
        $lstItems = $this->_getItems( $estructuraAgd[0], 0, 0 );

        if ( count( $lstItems ) > 0 ){
            $numReg = count( $estructuraAgd ) - 1;
            $this->_addItemHijos( $lstItems, $estructuraAgd, 1, $numReg );
        }
     ...
     *  @param type $idPadre     Id del owner
     * 
     *  @return type
     */
    private function _getItems( $estructura, $regOwner, $idPadre = 0 )
    {
        $tbItemsAgd = $this->getTable( 'ItemsAgd', 'MantenimientoTable' );
        $lstItemsEtr = $tbItemsAgd->getItemsByEtr( $estructura->idEstructura, $idPadre );
    ...
    /**
     *  
     *  Funcion recursiva que obtiene la lista de items hijos de una determinado 
     *  lista de items owner
     * 
     ...
     *
     */
    private function _addItemHijos( $lstItems, $estructuraAgd, $index, $numReg )
    {
        $estructura = $estructuraAgd[$index];

        foreach ( $lstItems AS $item ){
            $item->itemsHijos = $this->_getItems(   $estructura, 
                                                    $item->registroItem, 
                                                    $item->idItem );
            ...
            if ( count( $item->itemsHijos ) > 0 ){
                ...
                $this->_addItemHijos( $item->itemsHijos, $estructuraAgd, (int)$index );
            }
        }
    ...
    /**
     * 
     *  Guarda toda la data relacionada con una agenda
     * 
     *  @return type
     */
    ...
    private function _registrarAgenda( $dtaFrm )
    {
        ...
        $tbAgd =  $this->getTable('Agenda', 'MantenimientoTable');
    ...
    {
        $tbDetalle = $this->getTable('DetalleAgd', 'MantenimientoTable');
        $idDtall = null;
    ...
    {
        $tbEstr = $this->getTable('EstructuraAgd', 'MantenimientoTable');
        $idEstr = null;
    ...
    {
        $tbItem = $this->getTable('ItemsAgd', 'MantenimientoTable');
        $idItem = null;
    ...
    {
        $tbDetalleAgd = $this->getTable( 'Agenda', 'MantenimientoTable' );
