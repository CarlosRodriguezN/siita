 ...
 */
class mantenimientoModelNormas extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intCodigo_Norma', 
                                                'strDescripcion_norma'
                                                 );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=norma.edit&amp;id='. $item->intCodigo_Norma );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   nor.intCodigo_Norma,
                            nor.strDescripcion_norma,
                            nor.published,
                            u.name' );
        $query->from( '#__pei_tipo_norma nor' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
         $query->leftJoin( '#__users as u on u.id = nor.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( nor.published = 1 OR nor.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "nor.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " nor.strDescripcion_norma LIKE '" . $search . "'";
