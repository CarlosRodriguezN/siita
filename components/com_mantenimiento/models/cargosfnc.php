 ...
 */
class MantenimientoModelCargosFnc extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('strNombre_cargo');
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;view=cargoug&amp;layout=edit');
        }
    ...
    public function getListQuery() {
        ...
        $query->leftJoin('#__users as u on u.id = cf.checked_out');
        $query->where('ugc.published = 1');
        $query->where("cf.published = 1");
        $query->where("ugc.intCodigo_ug != 0");
        $query->order("ugc.strDescripcion_cargo ASC");
...
//
//        if ($published == '') {
//            $query->where('( cf.published = 1 OR cf.published = 0 )');
//        } else if ($published != '*') {
//            $published = (int) $published;
//            $query->where("cf.published = '" . $published . "'");
//        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " ugc.strDescripcion_cargo LIKE '" . $search . "' OR "
                    . "cf.strNombre_cargo LIKE '" . $search . "'";
    ...
    /**
     *  Retorna la lista de unidades de gestion
     * @return type
     */
    public function getLstUG()
    {
        $tbUG = $this->getTable('UnidadGestion', 'MantenimientoTable', $config = array());
