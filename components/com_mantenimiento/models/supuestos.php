 ...
 */
class mantenimientoModelSupuestos extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intCodigo_susp',
                                                'intCodigo_ojtoml',
                                                'strCodigocategoria',
                                                'strDescripcion_susp',
                                                'dteFecharegistro_susp',
                                                'dteFechamodificacion_susp'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=supuesto.edit&amp;intCodigo_susp=' . $item->intCodigo_susp);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    sup.intCodigo_susp, 
                            sup.intCodigo_ojtoml,
                            oml.strnombre_ojtoml,
                            sup.strCodigoCategoria,
                            cate.strDescripcionCategoria,
                            sup.strDescripcion_susp,
                            sup.dteFechaRegistro_susp,
                            sup.dteFechaModificacion_susp,
                            sup.published,
                            u.name');
        $query->from('#__gen_supuestos  as sup');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__gen_categoria as cate on cate.strCodigoCategoria = sup.strCodigoCategoria');
        $query->leftJoin('#__pfr_objetoml as oml on oml.intCodigo_ojtoml = sup.intCodigo_ojtoml');
        $query->leftJoin('#__users as u on u.id = sup.checked_out');
        ...
        if ($published == '') {
            $query->where('( sup.published = 1 OR sup.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("sup.published = '" . $published . "'");
        }
