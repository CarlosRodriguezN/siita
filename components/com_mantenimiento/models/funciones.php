...
class MantenimientoModelFunciones extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intCodigofuncion',
                                                'strDescripcion_funcion',
                                                'published' );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=funcion.edit&amp;intCodigofuncion='. $item->intCodigofuncion );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select('    fun.intCodigofuncion, 
                            fun.strDescripcion_funcion ,
                            fun.published,
                            u.name');
        $query->from('#__pei_funcion as fun');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = fun.checked_out');
        ...
        if( $published == '' ){
            $query->where( '( fun.published = 1 OR fun.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "fun.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " fun.strDescripcion_funcion LIKE '". $search ."'";
