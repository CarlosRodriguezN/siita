 ...
 */
class MantenimientoModelLineasBase extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'fun.strObservacion_fuente',
                                                'per.strdescripcion_per',
                                                'strDescripcion_lbind',
                                                'dteFechaInicio_lbind',
                                                'dteFechaFin_lbind'
                                             );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=lineabase.edit&amp;intCodigo_lbind=' . $item->intCodigo_lbind);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   lb.intCodigo_lbind,
                           lb.intCodigo_fuente,
                           lb.intCodigo_per,
                           lb.strDescripcion_lbind,
                           lb.dcmValor_lbind,
                           lb.dteFechaInicio_lbind,
                           lb.dteFechaFin_lbind,
                           lb.dteFechaRegistro_lbind,
                           fun.strObservacion_fuente,
                           per.strdescripcion_per,
                           lb.published,
                           u.name');
        ...
        $query->leftJoin('#__ind_fuente as fun on lb.intCodigo_fuente = fun.intCodigo_fuente');
        $query->leftJoin('#__gen_periodicidad as per on lb.intCodigo_per = per.intCodigo_per');
        $query->leftJoin('#__users as u on u.id = lb.checked_out');
        ...
        if ($published == '') {
            $query->where('( lb.published = 1 OR lb.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("lb.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " lb.strDescripcion_lbind LIKE '" . $search . "' OR"
                            ." per.strdescripcion_per LIKE '" . $search . "' OR"
                            . " fun.strObservacion_fuente LIKE '" . $search . "'";
