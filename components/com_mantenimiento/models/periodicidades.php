 ...
 */
class MantenimientoModelPeriodicidades extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'per.intcodigo_per',
                                                'per.idfecha',
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=periodicidad.edit&amp;intcodigo_per=' . $item->intcodigo_per);
        }
    ...
    /**
     *
     * Retorno informacion de la tabla "periodicidad".
     * 
     * @return Object 
     */
    public function getListQuery() {
        ...
        $query->select('   per.intcodigo_per,
                           per.idfecha,
                           per.strdescripcion_per,
                           per.published,
                           u.name');
        ...
        $query->leftJoin('#__users as u on u.id = per.checked_out');
        ...
        if ($published == '') {
            $query->where('( per.published = 1 OR per.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("per.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " per.strdescripcion_per LIKE '" . $search . "'";
