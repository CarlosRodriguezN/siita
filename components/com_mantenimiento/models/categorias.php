...
class mantenimientoModelCategorias extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'strCodigoCategoria',
                                                'intCodigo_ins',
                                                'strDescripcion_categoria',
                                                'strModuloSiita',
                                                'strSubModuloSiita',
                                                'published'
            );
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=categoria.edit&amp;strCodigoCategoria=' . $item->strCodigoCategoria);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   ca.strCodigoCategoria,
                           ca.intCodigo_ins,
                           ca.strDescripcion_categoria,
                           ca.strModuloSiita,
                           ca.strSubModuloSiita,
                           ins.strNombre_ins,
                           ca.published,
                           u.name');
        $query->from('#__gen_categoria as ca');
        ...
        $query->leftJoin('#__users as u on u.id = ca.checked_out');
        ...
        if (!empty($search)) {
            ...
            $field_searches = " ca.strDescripcion_categoria LIKE '" . $search . "' OR"
