 ...
 * Etapas Model
 */
class MantenimientoModelTiposGestion extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intIdTipoGestion_tpg',
                                                'published' );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=tipogestion.edit&amp;intIdTipoGestion_tpg='. $item->intIdTipoGestion_tpg );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select('    tpg.intIdTipoGestion_tpg, 
                            tpg.strDescripcion_tpg ,
                            tpg.strNombre_tpg ,
                            tpg.published,
                            u.name');
        $query->from('#__gen_tipo_gestion as tpg');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = tpg.checked_out');
        ...
        if( $published == '' ){
            $query->where( '( tpg.published = 1 OR tpg.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "tpg.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " tpg.strDescripcion_tpg LIKE '". $search ."'";
