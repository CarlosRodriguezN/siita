 ...
 */
class MantenimientoModelTposPlan extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intId_tpoPlan',
                                                'strdescripcion_per',
                                                'strAlias_tpoPlan',
                                                'published'
                                             );
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=tpoplan.edit&amp;intId_tpoPlan=' . $item->intId_tpoPlan);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   tp.intId_tpoPlan,
                           tp.strDescripcion_tpoPlan,
                           tp.strAlias_tpoPlan,
                           tp.published,
                           u.name');
        $query->from('#__pei_tipo_plan_ins as tp');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = tp.checked_out');
        ...
        if ($published == '') {
            $query->where('( tp.published = 1 OR tp.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("tp.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " tp.strDescripcion_tpoPlan LIKE '" . $search . "' OR"
                            ." tp.strAlias_tpoPlan LIKE '" . $search . "'";
