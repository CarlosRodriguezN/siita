 ...
 */
class MantenimientoModelEnfoques extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'nombrePadre',
                                                'nombreEnfoque',
                                                'simboloEnfoque'
                                             );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=enfoque.edit&amp;intId_enfoque=' . $item->idEnfoque);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   a.intId_enfoque      AS idEnfoque,
                           a.intId_enfoquePadre AS idPadre,
                           b.strNombre_enfoque  AS nombrePadre,
                           a.strNombre_enfoque  AS nombreEnfoque,
                           a.strSimbolo_enfoque AS simboloEnfoque,
                           a.published,
                           u.name');
        $query->from('#__gen_enfoque as a');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__gen_enfoque as b on b.intId_enfoque = a.intId_enfoquePadre ');
        $query->leftJoin('#__users as u on u.id = a.checked_out');
        $query->where('a.intId_enfoque > 0');
        ...
        if (!empty($search)) {
            ...
            $field_searches = " a.strNombre_enfoque LIKE '" . $search . "' OR"
                            ." b.strNombre_enfoque LIKE '" . $search . "' OR"
                            . " a.strSimbolo_enfoque LIKE '" . $search . "'";
