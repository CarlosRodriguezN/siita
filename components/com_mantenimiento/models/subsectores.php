 ...
 */
class mantenimientoModelSubsectores extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'inpCodigo_subsec', 
                                                'inpCodigo_sec',
                                                'strDescripcion_subsec',
                                                'strCodigo_subsec'
                                                 );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=subsector.edit&amp;id='. $item->inpCodigo_sec );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   subse.inpCodigo_subsec,
                            subse.inpCodigo_sec,
                            subse.strDescripcion_subsec,
                            subse.strCodigo_subsec,
                            subse.published,
                            se.strDescripcion_sec,
                            u.name' );
        $query->from( '#__gen_subsector as subse' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__gen_sector as se on se.inpCodigo_sec = subse.inpCodigo_sec' );
        $query->leftJoin( '#__users as u on u.id = subse.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( subse.published = 1 OR subse.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "subse.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " subse.strDescripcion_subsec LIKE '". $search ."' OR"
                            . " se.strDescripcion_sec LIKE '". $search ."'";
