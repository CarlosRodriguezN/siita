 ...
 */
class mantenimientoModelMediosverificaciones extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intCodigo_medver',
                                                'strCodigocategoria',
                                                'intCodigo_ojtoml',
                                                'strDescripcion_medver',
                                                'dteFecharegistro_medver',
                                                'dteFechamodificacion_medver'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=medioverificacion.edit&amp;intCodigo_medver=' . $item->intCodigo_medver);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    medVer.intCodigo_medver, 
                            medVer.intCodigo_ojtoml ,
                            medVer.strCodigocategoria ,
                            cate.strDescripcionCategoria,
                            objt.strnombre_ojtoml,
                            medVer.strDescripcion_medver ,
                            medVer.dteFecharegistro_medver ,
                            medVer.dteFechamodificacion_medver ,
                            medVer.published,
                            u.name');
        $query->from('#__gen_medio_verificacion  as medVer');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__pfr_objetoml as objt on objt.intCodigo_ojtoml = medVer.intCodigo_ojtoml');
        $query->leftJoin('#__gen_categoria as cate on cate.strCodigocategoria = medVer.strCodigocategoria');
        $query->leftJoin('#__users as u on u.id = medVer.checked_out');
        ...
        if ($published == '') {
            $query->where('( medVer.published = 1 OR medVer.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("medVer.published = '" . $published . "'");
        }
