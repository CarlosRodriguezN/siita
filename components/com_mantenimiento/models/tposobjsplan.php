 ...
 */
class MantenimientoModelTposObjsPlan extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intId_tpoObj',
                                                'strDescripcion_tpoObj',
                                                'strAliastipo_tpoObj',
                                                'published'
                                             );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=tpoobjplan.edit&amp;intId_tpoObj=' . $item->intId_tpoObj);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   topi.intId_tpoObj,
                           topi.strDescripcion_tpoObj,
                           topi.strAliastipo_tpoObj,
                           topi.published,
                           u.name');
        $query->from('#__pei_tipo_objetivo as topi');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = topi.checked_out');
        ...
        if ($published == '') {
            $query->where('( topi.published = 1 OR topi.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("topi.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " topi.strDescripcion_tpoObj LIKE '" . $search . "' OR"
                            ." topi.strAliastipo_tpoObj LIKE '" . $search . "'";
