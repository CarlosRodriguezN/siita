 ...
 */
class mantenimientoModelGrupos extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intCodigo_grp',
                                                'strDescripcion_grp');
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=grupo.edit&amp;intCodigo_grp=' . $item->intCodigo_grp);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    gr.intCodigo_grp, 
                            gr.strCodigoCategoria,
                            cate.strDescripcion_categoria,
                            gr.strDescripcion_grp ,
                            gr.published,
                            u.name');
        $query->from('#__gen_grupo as gr');

        //  Verificamos si algun registro esta siendo editado por algun usuario

        $query->leftJoin('#__gen_categoria as cate on cate.strCodigoCategoria = gr.strCodigoCategoria');
        $query->leftJoin('#__users as u on u.id = gr.checked_out');
        ...
        if ($published == '') {
            $query->where('( gr.published = 1 OR gr.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("gr.published = '" . $published . "'");
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " gr.strCodigoCategoria LIKE '" . $search . "' OR"
                            . "gr.strDescripcion_grp LIKE '" . $search . "'";
