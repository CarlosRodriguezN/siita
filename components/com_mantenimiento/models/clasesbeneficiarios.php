...
class mantenimientoModelClasesBeneficiarios extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('inpCodigo_benPart',
                                            'strDescripcion_benPart');
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=clasebeneficiario.edit&amp;inpCodigo_benPart=' . $item->inpCodigo_benPart);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    clasben.inpCodigo_benPart, 
                            clasben.strDescripcion_benPart ,
                            clasben.published,
                            u.name');
        $query->from('#__gen_clase_ben as clasben');

        //  Verificamos si algun registro esta siendo editado por algun usuario

        $query->leftJoin('#__users as u on u.id = clasben.checked_out');
        ...
        if ($published == '') {
            $query->where('( clasben.published = 1 OR clasben.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("clasben.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " clasben.strDescripcion_ag LIKE '" . $search . "' OR"
                            . " clasben.vigencia LIKE '" . $search . "'";
