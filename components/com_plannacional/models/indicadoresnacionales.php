...
class plannacionalModelIndicadoresnacionales extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intCodigo_in', 
                                                'intCodigoTipo_ind',
                                                'idCodigo_mn',
                                                'strDescripcion_in',
                                                'strFormula_in',
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   innac.intCodigo_in,
                            innac.intCodigoTipo_ind,
                            tiin.strDescripcionTipo_ind,
                            innac.idCodigo_mn,
                            mn.strDescripcion_mn,
                            innac.strDescripcion_in,
                            innac.strFormula_in,
                            innac.published,
                            u.name' );
        $query->from( '#__pnd_indicador_nacional as innac' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__ind_tipo_indicador as tiin on tiin.intCodigoTipo_ind = innac.intCodigoTipo_ind' );
        $query->leftJoin( '#__pnd_meta_nacional as mn on mn.idCodigo_mn = innac.idCodigo_mn' );
        $query->leftJoin( '#__users as u on u.id = innac.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( innac.published = 1 OR innac.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "innac.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " tiin.strDescripcionTipo_ind LIKE '". $search ."' OR"
                            . " mn.strDescripcion_mn LIKE '". $search ."' OR"
                            . " innac.strDescripcion_in LIKE '". $search ."'";
