 ...
 */
class plannacionalModelCategorias extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intCodigoCategoria', 
                                                'strCodigoCategoria',
                                                'intCodigo_ins',
                                                'strNombre_ins',
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=categoria.edit&amp;intCodigoCategoria='. $item->intCodigoCategoria );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   catego.intCodigoCategoria,
                            catego.strCodigoCategoria,
                            catego.intCodigo_ins,
                            gins.strNombre_ins,
                            catego.strDescripcionCategoria,
                            catego.strModuloSiita,
                            catego.strSubmoduloSiita,
                            catego.published,
                            u.name' );
        $query->from( '#__gen_categoria as catego' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__gen_institucion as gins on gins.intCodigo_ins = catego.intCodigo_ins' );
        $query->leftJoin( '#__users as u on u.id = catego.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( catego.published = 1 OR catego.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "catego.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " catego.strNombre_ins LIKE '". $search ."' OR"
                            . " catego.strDescripcionCategoria LIKE '". $search ."' OR"
                            . " catego.strModuloSiita LIKE '". $search ."' OR"
                            . " catego.strSubmoduloSiita LIKE '". $search ."'";
