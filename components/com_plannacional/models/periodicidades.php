 ...
 */
class plannacionalModelPeriodicidades extends JModelList 
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
         ...
         //  en este caso seria nombre, descripcion, estado( publicado / despublicado )
            $config['filter_fields'] = array(   'intCodigo_per', 
             'idFecha',
             'strDescripcion_per'
         );
      }
    ...
    public function getItems()
    {
      ...
      //  con esto mantenemos a nuestra vista ordenada
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=periodicidad.edit&amp;intCodigo_per='. $item->intCodigo_per );
      }
    ...
    */
    public function getListQuery()
    {
      ...
      $query = parent::getListQuery();
        ...
        $query->select( '   perio.intCodigo_per,
                            perio.idFecha,
                            perio.strDescripcion_per,
                            perio.published,
                            u.name' );
        $query->from( '#__gen_periodicidad as perio' );

      //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__users as u on u.id = perio.checked_out' );
      ...
      //
        ...
        if( $published == '' ){
            $query->where( '( perio.published = 1 OR perio.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "perio.published = '". $published ."'" );
      }
      ...
      $db = $this->getDbo();
        ...
        if( !empty ( $search ) ){
         ...
         //  en una sentencia Sql, con esto se previene de un ataque XSS
            ...
            $field_searches = " perio.strDescripcion_per LIKE '". $search ."' OR"
                            . " perio.idFecha LIKE '". $search ."'";
