 ...
 */
class plannacionalModelObjetivosnacionales extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intCodigo_on', 
                                                'intCodigo_pn',
                                                'strDescripcion_pn',
                                                'strDescripcion_on'
                                                 );
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=objetivonacional.edit&amp;intCodigo_on='. $item->intCodigo_on );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   obn.intCodigo_on,
                            obn.intCodigo_pn,
                            pln.strDescripcion_pn,
                            obn.strDescripcion_on,
                            obn.published,
                            u.name' );
        $query->from( '#__pnd_objetivo_nacional as obn' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__pnd_plannacional as pln on pln.intCodigo_pn = obn.intCodigo_pn' );
        $query->leftJoin( '#__users as u on u.id = obn.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( obn.published = 1 OR obn.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "obn.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " obn.intCodigo_on LIKE '". $search ."' OR"
                            . " obn.intCodigo_pn LIKE '". $search ."' OR"
                            . " obn.strDescripcion_on LIKE '". $search ."'";
