...
class plannacionalModelTiposindicador extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array(   'intCodigoTipo_ind', 
                                                'intCodigoCategoria',
                                                'strDescripcionCategoria',
                                                'strDescripcionTipo_ind',
                                                 );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=tipoindicador.edit&amp;intCodigoTipo_ind='.(int) $item->intCodigoTipo_ind );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   indica.intCodigoTipo_ind,
                            indica.intCodigoCategoria,
                            cate.strDescripcionCategoria,
                            indica.strDescripcionTipo_ind,
                            indica.published,
                            u.name' );
        $query->from( '#__ind_tipo_indicador as indica' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__gen_categoria as cate on cate.intCodigoCategoria = indica.intCodigoCategoria' );
        $query->leftJoin( '#__users as u on u.id = indica.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( indica.published = 1 OR indica.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "indica.published = '". $published ."'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " indica.strDescripcionCategoria LIKE '". $search ."' OR"
                            . " indica.strDescripcionTipo_ind LIKE '". $search ."'";
