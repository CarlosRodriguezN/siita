    ...
    jQuery('#saveObjetivoPoa').click(function(event) {
        ...
        if (validarObjetivo(data)) {
            if (tmpObjetivo == null) {
                data.idObjetivo = 0;
                data.idEntidad = 0;
                data.registroObj = objLstObjetivo.lstObjetivos.length + 1;
                data.published = 1;
                data.lstActividades = new Array();
                data.lstAcciones = new Array();
                addObjetivo(data);
                clCmpObjetivo();
                objLstObjetivo.lstObjetivos.push(data);
            }
            else {
...
function elmObjetivo(registroObjDel) {
    for (var j = 0; j < objLstObjetivo.lstObjetivos.length; j++) {
        if (objLstObjetivo.lstObjetivos[j].registroObj == registroObjDel) {
            objLstObjetivo.lstObjetivos[j].published = 0;
        }
    }
}
...
function actObjetivo(data) {
    for (var j = 0; j < objLstObjetivo.lstObjetivos.length; j++) {
        if (objLstObjetivo.lstObjetivos[j].registroObj == data.registroObj) {
            objLstObjetivo.lstObjetivos[j].idTpoObj = data.idTpoObj;
            objLstObjetivo.lstObjetivos[j].idPadreObj = data.idPadreObj;
            objLstObjetivo.lstObjetivos[j].descObjetivo = data.descObjetivo;
            objLstObjetivo.lstObjetivos[j].idPrioridadObj = data.idPrioridadObj;
            objLstObjetivo.lstObjetivos[j].nmbPrioridadObj = data.nmbPrioridadObj;
        }
    }
    clCmpObjetivo();
    reloadObjetivosTable();
...
function reloadObjetivosTable() {
    ...
    for (var j = 0; j < objLstObjetivo.lstObjetivos.length; j++) {
        if (objLstObjetivo.lstObjetivos[j].published == 1)
            addObjetivo(objLstObjetivo.lstObjetivos[j]);
    }
}
...
function getObjetivoByReg(registroObj) {
    var data = null;
    for (var j = 0; j < objLstObjetivo.lstObjetivos.length; j++) {
        if (objLstObjetivo.lstObjetivos[j].registroObj == registroObj) {
            data = objLstObjetivo.lstObjetivos[j];
        }
    }
    return data;
}
