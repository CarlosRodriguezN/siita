...
jQuery(document).ready(function() {
    var idRegGrafico = lstUbicacionesGeo.length;
    ...
    jQuery("#addGraficosObraTable").click(function() {
        ...
        if (flagGrafico != 0 || jQuery("#frmCoordenadasUG").css("display") == "block") {
            jConfirm("Se esta editando un gráfico, se perderá los cambios", "SIITA - ECORAE", function(result) {
                if (result) {
                    //  Cambio el texto del boton Editar
                    jQuery("#updGraf-" + flagGrafico).html("Editar");
                    flagGrafico = 0;
                    //  Limpia los furmularios
                    eliminarDataPuntos();
                    resetFrmUG();
                }
            });
    ...
    jQuery('#btnAddLstCoordenadas').click(function() {
        ...
        jQuery("#frmDtaUbcGeo").submit();
    });

    jQuery("#btnCancelarLstCoor").click(function() {
        //  Limpia los formularios y resetea las variables
        eliminarDataPuntos();
    });

    //  Gestiono el registro de puntos de una determinada coordenada.
    jQuery('#btnAddCoordenada').click(function() {
        //  Verifica si se ingresaron todos los campos
        if ( jQuery("#frmDtaUbcGeoCoordenada").valid() ) {
            var coordenada = [];
            ...
            if (flagVertice == 0) {
                ...
                coordenada["idCoordenada"] = jQuery('#jform_intId_cgcp').val();
            ...
            //  Limpio contenido de latitud y longitud
            linpiarFrmVertice();
        } else {
            jAlert('Campos OBLIGATORIOS', 'SIITA - ECORAE');
        }
    });
            ...
            if (countRegistros() > 0) {
                ...
                if (flagGrafico == 0) {
                    ...
                    var fila = '<tr id="' + idRegGraf + '">'
                            ...
                            + '     <td align="center" width="15" > <a id="updGraf-' + idRegGraf + '" class="updGrafico" >Editar</a> </td > '
                    ...
                    for (var j = 0; j < lstUbicacionesGeo.length; j++) {
                        if (lstUbicacionesGeo[j].idRegGrafico == flagGrafico) {
                            lstUbicacionesGeo[j].descGrafico = jQuery('#jform_strDescripcionGrafico_gcp').val();
    ...
    jQuery('.updGrafico').live('click', function() {
        ...
        if (flagGrafico != 0 || jQuery("#frmCoordenadasUG").css("display") == "block") {
            if (flagGrafico != idRegGrafico) {
                jConfirm("Se esta editando un gráfico, se perderá las modificaciones", "SIITA - ECORAE", function(result) {
                    if (result) {
    ...
    jQuery('.deleteCoordenada').live('click', function() {
        ...
        if (flagVertice != idCoor) {
            ...
            jAlert('El elemento se esta editando, no se puede eliminar', 'SITTA - ECORAE');
        }
    });
...
function resetFrmUG() {
    ...
    jQuery('#jform_strDescripcionGrafico_gcp').val('');
    
    jQuery("#frmDtaUbcGeo").validate().resetForm();
}
...
function linpiarFrmVertice() {
    ...
    jQuery("#frmDtaUbcGeoCoordenada").validate().resetForm();
}
    ...
    jQuery('#tbLstPuntos tr').each(function() {
        if (jQuery(this).attr('id') == idFilaCoor) {
            ...
            var fila = " <td align='center'>" + latitud + "</td>"
                    ...
                    + "     <td align='center' width='15' > <a id='updCoor-" + idFilaCoor + "' class='editCoordenada' >Editar</a> </td>"
...
function cargarDataUbcGeo(idRegGrafico) {
    ...
    jQuery("#jform_strDescripcionGrafico_gcp").val(lstUbicacionesGeo[indexArray].descGrafico);
    //
