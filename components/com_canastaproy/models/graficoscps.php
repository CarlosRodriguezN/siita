...
/**
 * Grafico de propuesta List Model
 */

class CanastaproyModelGraficoscps extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'strDescripcion_tg',
                                                'intIdPropuesta_cp',
                                                'strDescripcionGrafico_gcp',
                                                'published'
                                             );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_canastaproy&amp;task=graficocp.edit&amp;intId_gcp=' . $item->intId_gcp);
        }
    ...
    /**
     *
     * Retorno informacion de la tabla "grafico_propuesta".
     * 
     * @return Object 
     */
    public function getListQuery() {
        ...
        $query->select('   gp.intId_gcp,
                           gp.intId_tg,
                           tg.strDescripcion_tg,
                           gp.intIdPropuesta_cp,
                           pcp.strNombre_cp,
                           gp.strDescripcionGrafico_gcp,
                           gp.published,
                           u.name');
        $query->from('#__cp_grafico_propuesta as gp');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__gen_tipo_grafico as tg on tg.intId_tg = gp.intId_tg');
        $query->leftJoin('#__cp_propuesta as pcp on pcp.intIdPropuesta_cp = gp.intIdPropuesta_cp');
        $query->leftJoin('#__users as u on u.id = gp.checked_out');
        ...
        if ($published == '') {
            $query->where('( gp.published = 1 OR gp.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("gp.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " gp.strDescripcionGrafico_gcp LIKE '" . $search . "'";
