 ...
 */
class adminmapasModelEcoraeMapas extends JModelList
{
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_adminmapas&amp;task=ecoraemapa.edit&amp;intCodigo_wms=' . $item->intCodigo_wms );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select('   wme.intCodigo_wms,
                           wme.strNombre,
                           wme.strDescripcion,
                           wme.strCopyright,
                           wme.published');
        $query->from( '#__map_wms as wme' );
        $query->where( 'wme.intTpoMapa = 0' );
