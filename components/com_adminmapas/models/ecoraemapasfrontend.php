 ...
 */
class adminmapasModelEcoraeMapasFrontEnd extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array('intCodigo_shp_ecorae',
                'strNombre',
                'strDescripcion'
            );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_adminmapas&amp;task=ecoraemapafrontend.edit&amp;intCodigo_shp_ecorae=' . $item->intCodigo_shp_ecorae);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   wme.intCodigo_shp_ecorae,
                           wme.strNombre,
                           wme.strDescripcion,
                           wme.strCopyright,
                           wme.published');
        $query->from('#__map_wms_shp_ecorae AS wme');

        //$query->leftJoin('#__gen_institucion as ins on ins.intCodigo_ins = ca.intCodigo_ins');
        //
        ...
        if ($published == '') {
            $query->where('( wme.published = 1 OR wme.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("wme.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " wme.strNombre LIKE '" . $search . "' AND"
                    . " wme.strDescripcion LIKE '" . $search . "'";
