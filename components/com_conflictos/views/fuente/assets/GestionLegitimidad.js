jQuery(document).ready(function() {
    ...
    jQuery('#addLeg').click(function () {
        habilitarBtns( 2, "#saveLegitimidadFuente" );
        resetForm( true );
        idLegitimidad = 0;
    });
    ...
    jQuery('#cancelLeg').click(function () {
        ...
        habilitarBtns( 1, "#saveLegitimidadFuente" );
    });
    ...
    function guardarLegitimidad()
    {
        ...
        jQuery.ajax({
            ...
            if ( Array.isArray( dataInfo ) ){
                ...
                resetForm( false );
                if ( idLegitimidad != 0 ) {
                    updLstLgiFuente( idLegitimidad, dataInfo );
     ...
     * @returns {undefined}
     */
    function updLstLgiFuente( id, list ){
        var reg = -1;
        for (var i = 0; i < list.length; i++) {
            if ( list[i].id == id ) {
                reg = i;
        ...
        if ( reg != -1 ) {
            for ( var j = 0; j < oFuente.lstLegitimidad.length; j++ ) {
                if ( oFuente.lstLegitimidad[j].idLegitimidad == id )
                    oFuente.lstLegitimidad[j].nmbLegitimidad = list[reg].nombre;
            }
        }
    }
