...
class ConflictosViewFuente extends JView
{
    ...
    public function display( $tpl = null )
    {
        ...
        if ( $item->published == 0 && $item->intId_fte != 0  ){
    ...
    protected function getToolbar()
    {
        ...
        if( $this->canDo->get( 'core.edit' ) || $this->canDo->get( 'core.create' ) ){
            $bar->appendButton( 'Standard', 'save', JText::_( 'BTN_GUARDAR' ), 'fuente.save', false );
            $bar->appendButton( 'Standard', 'save', JText::_( 'BTN_GUARDAR_SALIR' ), 'fuente.saveExit', false );
        }

        
        if( $this->availableDelete() ){
            $bar->appendButton( 'Standard', 'delete', JText::_( 'BTN_ELIMINAR' ), 'fuente.delete', false );
        }

        $bar->appendButton( 'Separator' );

        $bar->appendButton( 'Standard', 'list', JText::_( 'BTN_LISTAR_FUENTES' ), 'fuente.list', false );
        $bar->appendButton( 'Standard', 'cancel', JText::_( 'BTN_CANCELAR' ), 'fuente.cancel', false );
    ...
    {
        $isNew = ($this->item->intId_fte > 0);
        $document = JFactory::getDocument();

        $document->setTitle( $isNew ? JText::_( 'COM_CONFLICTOS_FUENTE_CREATING' ) : JText::_( 'COM_CONFLICTOS_FUENTE_EDITING' )  );
        ...
        // adjunto la clase java script de un fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/common.js' );

        // adjunto la clase java script de un reglas 
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/ruler.js' );

        // adjunto la clase java script de un fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/ajaxCalls.js' );

        // adjunto la clase java script de un fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/Fuente.js' );

        // adjunto la clase java script de un incidencia de una fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/IncidenciaFuente.js' );

        // adjunto la clase java script de un incidencia de una fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/LegitimidadFuente.js' );

        // adjunto la clase java script de un llamadas ajax
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/ajaxCalls.js' );

        // adjunto la clase java script de un almacenamiento
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/save.js' );

        // adjunto la clase java script de un fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/UnidadTerritorial.js' );
        
        // adjunto la clase java script para la gestion de un tipo de fuente
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/GestionTpoFuente.js' );
        
        // adjunto la clase java script para la gestion de Incidencia
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/GestionIncidencia.js' );
        
        // adjunto la clase java script para la gestion de Legitimidad
        $document->addScript( JURI::root() . 'components/com_conflictos/views/fuente/assets/GestionLegitimidad.js' );

        //  Adjunto lista de Arreglos de Sub contratos
        $document->addScriptDeclaration( $this->_scriptFuente(), $type = 'text/javascript' );

        JText::script( 'COM_CONFLICTOS_TEMA_ERROR_UNACCEPTABLE' );
    }

    /**
     * Arma el JavaScript de el fuente 
     * @return string
     */
    private function _scriptFuente()
    {
        $oFuente = $this->fuente;
        $script = "";
        $script .="var oFuente = new cFuente();";
        if( $this->item->intId_fte != 0 ) {
            $fuenteJSON = json_encode( $oFuente );
            $script .='oFuente.setDataGeneral(' . $fuenteJSON . ');';
            if( count( $oFuente->lstIncidencias ) > 0 ) {
                $lstIncidenciaJSON = json_encode( $oFuente->lstIncidencias );
                $script .='oFuente.setlstIncidencia(' . $lstIncidenciaJSON . ');';
            }
            if( count( $oFuente->lstLegitimidad ) > 0 ) {
                $lstLegitimidadJSON = json_encode( $oFuente->lstLegitimidad );
                $script .='oFuente.setlstLegitimidad(' . $lstLegitimidadJSON . ');';
            }
            if( count( $oFuente->unidadTerrirorial ) > 0 ) {
                $unidadTerrirorial = json_encode( $oFuente->unidadTerrirorial );
                $script .='oFuente.setUnidadTerritorial(' . $unidadTerrirorial . ');';
            }
        }
        return $script;
    }
