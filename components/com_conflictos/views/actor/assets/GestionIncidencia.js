jQuery(document).ready(function() {
    ...
    jQuery('#addInc').click(function () {
        habilitarBtns( 2, "#saveIncidenciaActor" );
        resetForm( true );
        idIncidencia = 0;
    });
    
    /**
     *  Cancela la gestion de un nuevo registro
     */
    jQuery('#cancelInc').click(function () {
        resetForm( false );
        jQuery('#jform_idIncidenciaTxt').val('');
        habilitarBtns( 1, "#saveIncidenciaActor" );
    });
    ...
    function guardarIncidencia()
    {
        ...
        jQuery.ajax({
            ...
            if ( Array.isArray( dataInfo ) ){
                ...
                resetForm( false );
                if ( idIncidencia != 0 ) {
                    updLstInsActor( idIncidencia, dataInfo );
     ...
     * @returns {undefined}
     */
    function updLstInsActor( id, list ){
        var reg = -1;
        for (var i = 0; i < list.length; i++) {
            if ( list[i].id == id ) {
                reg = i;
        ...
        if ( reg != -1 ) {
            for ( var j = 0; j < oActor.lstIncidencias.length; j++ ) {
                if ( oActor.lstIncidencias[j].idIncidencia == id )
                    oActor.lstIncidencias[j].nmbIncidencia = list[reg].nombre;
            }
        }
    }
