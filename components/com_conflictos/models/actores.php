 ...
 */
class ConflictosModelActores extends JModelList
{
    ...
    public function __construct( $config = array( ) )
    {
        if( empty( $config['filter_fields'] ) ) {
            ...
            $config['filter_fields'] = array( 'intId_act',
                'strNombre_act',
                'strApellido_act',
                'strCorreo_act'
            );
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ) {
            $item->url = JRoute::_( 'index.php?option=com_conflictos&amp;task=actor.edit&amp;intId_act=' . $item->intId_act );
        }
        return $items;
    }
...
//            $query->where( '( ca.published = 1 OR ca.published = 0 )' );
        ...
        if( !empty( $search ) ) {
            ...
            $field_searches = " ca.strNombre_act LIKE '" . $search . "' OR"
                    . " ca.strApellido_act LIKE '" . $search . "'";
