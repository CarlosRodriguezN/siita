 ...
 * 
 */
class ConflictosTableVwdpa extends JTable {
     ...
     * 
     * @param type $idTema Identificador de unidad territorial
     * 
     * @return type
     */
    public function getLstUndTerritoriales($idTema) {
        try {
            $db = JFactory::getDBO();

            $sql = "SELECT  t1.intID_ut             AS idUndTerritorial, 
                            t2.intID_tut            AS tpoUndTerritorial
                    FROM " . $db->nameQuote("#__gc_tema_uterritorial") . " t1
                    JOIN " . $db->nameQuote("#__ut_undTerritorial") . " t2 ON
                    t1.intID_ut = t2.intID_ut
                    WHERE t1.intId_tma = '" . $idTema . "'";
            $db->setQuery((string) $sql);
            $db->query();

            if( $db->getNumRows() > 0 ) {
                $lstUT = $db->loadObjectList();
                $dtaUT =$this->getDataUnidadTerritorial( $lstUT );
            }
     ...
     public function getUndTerritorialesFuente($idFuente) {
        try {
            $db = JFactory::getDBO();

            $sql = "SELECT  t1.intID_ut             AS idUndTerritorial, 
                            t2.intID_tut            AS tpoUndTerritorial
                    FROM " . $db->nameQuote("#__gc_fuente") . " t1
                    JOIN " . $db->nameQuote("#__ut_undTerritorial") . " t2 ON
                    t1.intID_ut = t2.intID_ut
                    WHERE t1.intId_fte = '" . $idFuente . "'";
            $db->setQuery((string) $sql);
            $db->query();
    ...
    public function getDataUnidadTerritorial( $lstUT )
    {
        foreach( $lstUT as $ut ) {
