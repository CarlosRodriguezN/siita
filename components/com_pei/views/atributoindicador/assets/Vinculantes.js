jQuery( document ).ready( function(){
    ...
    jQuery( '#jform_intIdTpoUndMedida' ).change(function( event, idUndMedida ){
        ...
        jQuery.ajax({   type: 'GET',
                        ...
                        error : function( jqXHR, status, error ) {
                            alert( COM_PEI_GESTION_ENTIDAD + ' ' + error +' '+ jqXHR +' '+ status );
                        }
        }).complete( function( data ){
                var dataInfo = eval( data.responseText );
                var numRegistros = dataInfo.length;
    ...
    jQuery( '#jform_intIdUGResponsable' ).live( 'change', function( event, idResponsable ){
        ...
        jQuery.ajax({   type: 'POST',
                        ...
                        error : function( jqXHR, status, error ) {
                            alert( 'PEI - Gestion Atributos Indicador: ' + error +' '+ jqXHR +' '+ status );
                        }
        }).complete( function( data ){
            var dataInfo = eval( data.responseText );
            var numRegistros = dataInfo.length;
...
//
    jQuery( '#jform_idTpoEntidad' ).change( function( event, idEntidad ){
        ...
        jQuery('#jform_idEntidad').html( '<option value="0">'+ COM_PEI_CARGANDO +'</option>' );
...
//
    ...
    jQuery( '#jform_idPlantilla' ).live( 'change', function(){
        ...
        jQuery.ajax({   type: 'POST',
            ...
            recorrerCombo( jQuery('#jform_intIdUGResponsable option'), dataInfo.idUGResponsable );
            jQuery('#jform_intIdUGResponsable').trigger( 'change', dataInfo.idResponsable );
