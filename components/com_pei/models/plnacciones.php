...
class PeiModelPlnAcciones extends JModelList {
    ...
    public function getListQuery() {
        ...
        if ($published == '') {
            $query->where('( pi.published = 1 OR pi.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("pi.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " pi.strDescripcion_pi LIKE '" . $search . "' OR"
                            . " pi.strDescripcion_tpoPlan LIKE '" . $search . "'";
