...
class PeiModelPeis extends JModelList {
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'intId_pi',
                                                'intId_tpoPlan',
                                                'intIdPadre_pi',
                                                'intCodigo_ins',
                                                'strDescripcion_pi',
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_pei&amp;task=pei.edit&amp;intId_pi=' . $item->intId_pi);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('    pi.intId_pi,
                            pi.intId_tpoPlan,
                            pi.intCodigo_ins,
                            pi.strDescripcion_pi,
                            pi.dteFechainicio_pi,
                            pi.dteFechafin_pi,
                            pi.blnVigente_pi,
                            pi.strAlias_pi,
                            pi.published,
                            u.name');
        $query->from('#__pln_plan_institucion as pi');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = pi.checked_out');
        //  Filtramos por el tipo de plan, el tipo de plan estrastegico institucional igual a 4
        $query->where('pi.intId_tpoPlan = 1');
        $query->where('pi.published = 1');
        ...
        if (!empty($search)) {
            ...
            $field_searches = " pi.strDescripcion_pi LIKE '" . $search . "'";
        ...
        if ($orderCol != '') {
            ...
            $query->order( 'pi.blnVigente_pi DESC' );
        }
