 ...
 * Etapas Model
 */
class ProyectosModelProgramas extends JModelList
{
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_proyectos&amp;task=programa.edit&amp;intcodigo_prg='. $item->intcodigo_prg );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   t1.intcodigo_prg,
                            UPPER( t3.strdescripcion_te ) AS tipoEntidad,
                            t1.strdescripcion_prg,
                            t1.published,
                            u.name' );
        $query->from( '#__pfr_programa t1' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__gen_entidad t2 ON t1.intIdEntidad_ent = t2.intidentidad_ent' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__gen_tipo_entidad t3 ON t2.intidtipoentidad_te = t3.intidtipoentidad_te' );
