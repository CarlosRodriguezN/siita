...
jQuery(document).ready(function() {
    ...
    function agregarFilaObjetivo(objetivo)
    {
        ...
        var fila = makeFilaTable(objetivo, 0);
    ...
    function actualizarFila(objetivo)
    {
        ...
        jQuery('#tbLstObjetivos tr').each(function() {
            if (jQuery(this).attr('id') == flagObjetivo) {
                ...
                //  Construyo la Fila
                var fila = makeFilaTable(objetivo, 1);
     ...
     * @param {type} objetivo
     * @param {type} task
     * @returns {String}Arma el html de un fila de la tabla de objetivos
     */
    function makeFilaTable(objetivo, task)
    {
        var idReg = objetivo.registroObj;
        var fila = '';
        var ds;
        ...
        fila += '     <td align="center" style="padding-left: 22px; width: 30px;" >';
        ...
        ds = objetivo.semaforizacionInd(2);
        fila += '     <td align="center" width="15" >';
        fila += '        <a onclick="SqueezeBox.fromElement( \'index.php?option=com_indicadores&view=indicadores&layout=edit&tpoIndicador=programa&idRegObjetivo=' + idReg + '&tmpl=component&task=preview\', {size:{x:'+ POPUP_IND_ANCHO +',y:'+ POPUP_IND_ALTO +'  }, handler:\'iframe\'} );">';
