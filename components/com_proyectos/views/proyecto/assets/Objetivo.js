 ...
 * 
 */
Objetivo.prototype.semaforizacionInd = function( tpoInd )
{
    ...
    if( nrInd > 0 ){
        for( var x = 0; x < nrInd; x++ ){
            dtaSemaforizacion = '';
            var objInd;

            if( typeOf( lstIM[x] ) == "object" ){
                var objInd = new Indicador();
                objInd.setDtaIndicador( lstIM[x] );
            }else{
                objInd = lstIM[x];
            }

            if( parseInt( objInd.idTpoIndicador ) == tpoInd ){
