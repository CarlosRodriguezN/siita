...
class JFormFieldEstadoEntidad extends JFormFieldList
{
    ...
    */
    protected function getInput()  
    {
        ...
        $idEstadoEntidad = ( $idPrograma > 0 )  ? $this->_getIdEstadoEntidad( $idPrograma )
                                                : 1;
        $db = JFactory::getDBO();
        ...
        $options .= '<option value="0">'. JText::_( 'COM_PROGRAMA_FIELD_ESTADO_ENTIDAD_TITLE' ) .'</option>';
     ...
     * 
     * @param int $idPrograma  Identificador de un proyecto
     * 
     * @return int
     * 
     */
    private function _getIdEstadoEntidad( $idPrograma )
    {
        $db = JFactory::getDBO();
        $query = $db->getQuery(true);
        ...
        $query->where( 'ta.dtFchRegistro_ee = ( SELECT  MAX( t2.dtFchRegistro_ee ) as fchRegistro
                                                FROM tb_pfr_programa t1 INNER JOIN tb_gen_estado_entidad t2 ON t1.intIdEntidad_ent = t2.intIdEntidad_ent 
                                                WHERE t1.intCodigo_prg = '. $idPrograma .' ) ' );
        
        $db->setQuery( ( string )$query );  
        $dtaPrograma = $db->loadObject();
        
        return $dtaPrograma->idEstadoEntidad;
    }
