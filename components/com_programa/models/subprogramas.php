 ...
 *  Modelo Programas
 */
class ProgramaModelSubProgramas extends JModelList
{
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_programa&amp;task=subprograma.edit&amp;intId_SubPrograma='. $item->intId_SubPrograma );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   t1.intId_SubPrograma AS idSubPrograma,
                            t1.strCodigoSubPrograma AS codigoSubPrograma,
                            t1.strDescripcion AS descripcion,
                            t1.strAlias as alias,
                            t1.published,
                            u.name' );
       ...
       // $query->leftJoin( '#__gen_entidad t2 ON t1.intIdEntidad_ent = t2.intidentidad_ent' );
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches =   "   t1.descripcion LIKE '". $search ."' OR "
                                ."  t1.alias LIKE '". $search ."' OR"
                                ."  t1.codigoSubPrograma LIKE '". $search ."'";
