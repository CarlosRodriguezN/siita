 ...
 */
class ActividadViewActividadesByUG extends JView {
    ...
    public function display($tpl = null) {
        ...
        $mdActByUG = $this->getModel();
        $this->lstActividadesByFnc = $mdActByUG->getActByFnc($idUG);
    ...
    protected function getToolbar() {
        ...
        $bar->appendButton('Standard', 'cancel', 'Salir', 'actividadesbyug.salir', false);
    ...
    protected function setDocument() {
        ...
        //  Adjunto libreria para el lenguaje de activiades
        $document->addScript(JURI::root() . 'components/com_actividad/views/actividadesbyug/assets/ES_ACT.js');

        //  Adjunto libreria para las reglas de los input
        $document->addScript(JURI::root() . 'components/com_actividad/views/actividadesbyug/assets/ReglasAct.js');

        //  Adjunto libreria para la clase Funcionario
        $document->addScript(JURI::root() . 'components/com_actividad/views/actividadesbyug/assets/Funcionario.js');

        //  Adjunto libreria para la cargas de Actividades de un funcionario
        $document->addScript(JURI::root() . 'components/com_actividad/views/actividadesbyug/assets/ActividadesByFnc.js');

        //  Adjunto libreria que muestra las actividades
        $document->addScript(JURI::root() . 'components/com_actividad/views/actividadesbyug/assets/Load.js');

        //  Adjunto el script para la carga de la lista de actividades por funcionario
        $document->addScriptDeclaration( $this->_getLstActsByFnc(), $type = 'text/javascript' );
        
        //  Adjunto lista de Arreglos para gestion de arrays
        JText::script('COM_ACTIVIDAD_PLAN_ERROR_UNACCEPTABLE');
    }

    private function _getLstActsByFnc()
    {

        $retval = '';

        //  Lista de Poas de una Unidad de Gestión
        $retval .= 'objLstActFnc = new ActividadesByFnc();';
        
        //  lista de poas de la unidad de gestion 
        if( $this->lstActividadesByFnc ){
            foreach( $this->lstActividadesByFnc as $fnc ){
                $retval .= 'var objFnc = new Funcionario();';
                $retval .= 'objFnc.setDtaFnc( ' . json_encode( $fnc ) . ' );';
                $retval .= 'objLstActFnc.addFnc( objFnc );';
            }
        }
