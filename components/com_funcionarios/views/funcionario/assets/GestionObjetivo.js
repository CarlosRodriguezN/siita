jQuery(document).ready(function() {
    ...
    /**
     *  Desabilita la opcion de agregar objetivos en el caso de que no haya un plan selecionado
     */
            ...
            if ( flagObjetivo === -1  ) {
                ...
                for (var i = 0; i < numReg; i++) {
                    if( objLstPoas.lstPoas[idRegPoa].lstObjetivos[i].registroObj === flagObjetivo ){
                        ...
                        actualizarFila( objLstPoas.lstPoas[idRegPoa].lstObjetivos[i], idRegPoa );
                    }
                }
            }
    ...
    function makeFila( objetivo, idPoa, op )
    {
        ...
        var idFncUG = jQuery("#jform_intId_ugf").val();
        ...
        fila += '     <td align="left">' + objetivo.descObjetivo + '</td>';
        fila += '     <td align="center" style="vertical-align: middle;">' + objetivo.nmbPrioridadObj + '</td>';

        //  Alineacion de un Objetivo
        fila += '     <td align="center" width="15" style="padding-left: 8px; vertical-align: middle;">';
        fila += '        <a onclick="SqueezeBox.fromElement( \'index.php?option=com_alineacion&view=operativa&tpoEntidad=11&layout=edit&registroPoa=' + idPoa + '&registroObj=' + idReg + '&tmpl=component&task=preview\', {size:{x:' + POPUP_ANCHO + ',y:' + POPUP_ALTO + '}, handler:\'iframe\'} );">';
        fila += '           <div class="sprite-alineacion" id="smfAln-' + idReg + '" title="' + JSL_ALINEACION + '"/>';
        fila += '        </a>';
        fila += '     </td>';

        //  Actividades de un Objetivo
        fila += '     <td align="center" width="15" align="center" style="padding-left: 10px;width: 20px; vertical-align: middle;">';
        fila += '        <a onclick="SqueezeBox.fromElement( \'index.php?option=com_actividad&view=actividad&layout=edit&registroPoa=' + idPoa + '&registroObj=' + idReg + '&tmpl=component&task=preview\', {size:{x:' + POPUP_ANCHO + ',y:' + POPUP_ALTO + '}, handler:\'iframe\'} );">';
        fila += '            <div class="sprite-actividades" id="smfAct-' + idReg + '" title="' + JSL_ACTIVIDADES + '"/>';
        fila += '        </a>';
        fila += '     </td>';

        //  Acciones de un objetivo
        fila += '     <td align="center" width="15" align="center" style="padding-left: 10px;width: 20px; vertical-align: middle;">';
        fila += '        <a onclick="SqueezeBox.fromElement( \'index.php?option=com_accion&view=plnaccion&layout=edit&idFnc=' + idFncUG + '&idUG=' + idUG + '&registroPln=' + idPoa + '&tpoPln=2&registroObj=' + idReg + '&tmpl=component&task=preview\', {size:{x:' + POPUP_ANCHO + ',y:' + POPUP_ALTO + '}, handler:\'iframe\'} );">';
        fila += '            <div class="sprite-acciones" id="smfAcc-' + idReg + '" title="' + JSL_ACCIONES + '"/>';
        fila += '        </a>';
        fila += '     </td>';
        
        //  Indicadores del Objetivo ( Meta, Intermedios, Apoyo, etc )
        var ds = objetivo.semaforizacionInd();
        fila += '     <td id="' + idReg + '" align="center" style="padding-left: 10px;width: 20px; vertical-align: middle;" >';
        fila += '        <a onclick="SqueezeBox.fromElement( \'index.php?option=com_indicadores&view=indicadores&layout=edit&tpoPlan=poaF&tpo='+ objetivo.idTpoObj +'&tpoIndicador=fun&idPlan=' + idPoa + '&idRegObjetivo=' + idReg + '&idUsr='+ idUsr +'&tmpl=component&task=preview\', {size:{x:' + POPUP_IND_ANCHO + ',y:' + POPUP_IND_ALTO + '}, handler:\'iframe\'} );">';
     ...
     * 
     * @param {type} idPoaObj       Id del Poa-Objetivo en el caso de una nueva edicion, si no es -1
     * @param {type} frm            Opcion de tarea, True para habilitar el formulario y false para deshabilitarlo
     * @returns {undefined}
     */
    ...
    function avalibleDel(data)
    {
        ...
        if ( numReg > 0) {
            for (var i = 0; i < numReg; i++) {
                if ( parseInt( data[i].published ) === 1)
                    result = false;
            }
        }
        return result;
    ...
    function delFila(idFila, tabla)
    {
        ...
        jQuery(tabla + ' tr').each(function() {
            if ( parseInt( jQuery(this).attr('id') ) === idFila ){
                jQuery(this).remove();
            }
        });
    }
    ...
    function confirmUpdObj( idPoa, idObj )
    {
        ...
        for (var i = 0; i < lstObjsPoa.length; i++) {
            if ( parseInt( lstObjsPoa[i].registroObj ) === parseInt( idObj ) 
                 && ( parseInt( jQuery("#jform_intPrioridad_ob :selected").val() ) !== parseInt( lstObjsPoa[i].idPrioridadObj )
                      || jQuery("#jform_strDescripcion_ob").val() !== lstObjsPoa[i].descObjetivo ) ) {
                    resultado = true;
            }
        }
    ...
    function cargarObjetivos( idRegPoa )
    {
        ...
        if (    typeof (lstObjetivosPoa) !== "undefined" 
                && lstObjetivosPoa.length > 0 
                && !(avalibleDel(lstObjetivosPoa ) ) ){
            jQuery("#srObj").css("display", "none");
            for ( var i = 0; i < lstObjetivosPoa.length; i++ ) {
                if ( parseInt(lstObjetivosPoa[i].published) === 1 ) {

                    jQuery( '#idLstPOAsFUG' ).html( getAnioVigencia( lstObjetivosPoa[i].fchInicioPlan ) );
                    ...
                    agregarFila( objObjetivo, idRegPoa );
                    validateSemaforoObjetivo(i, 2, idRegPoa);
                }
            }
        } else {
            jQuery("#srObj").css("display", "block");
...
function getAnioVigencia( fchInicioPlan )
{
    ...
    if( typeOf( fchInicioPlan ) !== "null" ){
        ...
        msgAnio = COM_FUNCIONARIOS_FIELD_OBJETIVOS_FNC_LABEL + objFecha.getFullYear() + '&nbsp;';
    }
...
function validateSemaforoObjetivo(reg, tpoPln, idRegPln){
    ...
    } else if ( typeof (tpoPln) !== "undefined" 
                && parseInt( tpoPln ) === 2 ){
...
function semaforoAlineacion(reg, tpoPln, idRegPln) 
{
    ...
    for(var i=0; i<lstAlin.length; i++){
       if ( parseInt( lstAlin[i].published ) === 1){
           numReg = ++numReg;
       } 
    }
...
function semaforoActividades( reg, tpoPln, idRegPln )
{
    ...
    for(var i=0; i<lstAct.length; i++){
       if ( parseInt( lstAct[i].published ) === 1 ){
           numAct = ++numAct;
       } 
    }
