...
class jTableIndicador extends JTable
{
    /**
    * Constructor
    *
    ...
    */
    ...
    public function getDataIndicadores( $idEntidad )
    {
        try {
            ...
            $query->select( '   DISTINCT t1.intIdIndEntidad                                                                             AS idIndEntidad,
                                ...
                                t7.strSimbolo_unimed                                                                                    AS simbolo,
                                t2.intCodigoTipo_ind                                                                                    AS idTpoIndicador,
                                t12.strDescripcionTipo_ind                                                                              AS tpoIndicador,
                                ...
                                t11.strdescripcion_per                  AS frecuenciaMonitoreo,
                                ...
                                t2.strMetodologiaCalculo_ind            AS metodologia,
                                t2.strLimitacionesTecnicas_ind          AS limitaciones,
                                t2.strInterpretacionIndicador_str       AS interpretacion,
                                t2.strDisponibilidadIndicador_ind       AS disponibilidad,
                                ...
                                t2.dteFechaRegistro_ind                                                             AS fechaElaboracion,
                                t2.dteFechaModificacion_ind                                                         AS fechaModificacion,
                                t1.intSenplades_indEnt                                                              AS senplades ' );
            ...
            $query->join( 'INNER', '#__ind_tipo_indicador t12 ON t2.intCodigoTipo_ind = t12.intCodigoTipo_ind' );

            if( is_null( $idEntidad ) == false ){
    ...
    public function getLstOtrosIndicadores( $idEntidad, $idEntFnc = null )
    {
        try {
            ...
            $query->leftJoin( '#__ind_funcionario_responsable t9 ON t9.intIdIndEntidad = t1.intIdIndEntidad AND t9.intVigencia_fgr = 1' );
            $query->leftJoin( '#__gen_ug_funcionario t10 ON t10.intId_ugf = t9.intId_ugf' );
            $query->join( 'INNER', '#__gen_funcionario t11 ON t10.intCodigo_fnc = t11.intCodigo_fnc' );
            $query->join( 'INNER', '#__ind_tipo_indicador t12 ON t2.intCodigoTipo_ind = t12.intCodigoTipo_ind' );
    ...
    public function getLstIndContexto( $idEntidad )
    {
        try {
            ...
            $query->select( '   DISTINCT 
                                    ...
                                    t10.intCodigo_ug                                        AS idResponsableUG,
                                    t2.intIdDimension_ind                                   AS idDimIndicador,
                                    t1.strAccesoTableu_indEnt                               AS nombreReporte' );
            $query->from( '#__ind_indicador_entidad t1' );
            $query->join( 'INNER', '#__ind_indicador t2 ON t1.intCodigo_ind = t2.intCodigo_ind' );
            $query->leftJoin( '#__gen_unidad_analisis t5 ON t5.inpCodigo_unianl = t2.inpCodigo_unianl' );
            $query->leftJoin( '#__gen_unidad_medida t7 ON t7.intCodigo_unimed = t2.intCodigo_unimed' );
...
/////////////////////////////////////////////////////////
    ...
    {
        $dtaIndicador["intCodigo_ind"]          = ( is_null( $indicador->idIndicador ) )
                                                    ? 0 
                                                    : $indicador->idIndicador;

        $dtaIndicador["intCodigoTipo_ind"]      = ( is_null( $indicador->idTpoIndicador ) ) 
                                                    ? 0 
                                                    : $indicador->idTpoIndicador;

        $dtaIndicador["inpCodigo_claseind"]     = ( is_null( $indicador->idClaseIndicador ) )   
                                                    ? 0 
                                                    : $indicador->idClaseIndicador;

        $dtaIndicador["intCodigo_unimed"]       = ( is_null( $indicador->idUndMedida ) )
                                                    ? 0 
                                                    : $indicador->idUndMedida;

        $dtaIndicador["inpCodigo_unianl"]       = ( is_null( $indicador->idUndAnalisis ) )  
                                                    ? 0 
                                                    : $indicador->idUndAnalisis;

        $dtaIndicador["strNombre_ind"]          = ( is_null( $indicador->nombreIndicador ) )
                                                    ? 0 
                                                    : $indicador->nombreIndicador;

        $dtaIndicador["strDescripcion_ind"]     = ( is_null( $indicador->descripcion ) )
                                                    ? 0 
                                                    : $indicador->descripcion;

        $dtaIndicador["strFormula_ind"]         = ( strlen( $indicador->formula ) == 0 )
                                                    ? $indicador->nombreIndicador . ' = ' . $indicador->nombreIndicador
                                                    : $indicador->formula;

        $dtaIndicador["intIdMetodoCalculo_ind"] = ( is_null( $indicador->idMetodoCalculo ) )
                                                    ? 1 
                                                    : $indicador->idMetodoCalculo;

        $dtaIndicador["inpCategoria_ind"]       = ( is_null( $indicador->idCategoria ) )
                                                    ? 0 
                                                    : $indicador->idCategoria;

        $dtaIndicador["intIdDimension_ind"]     = ( is_null( $indicador->idDimension ) )
                                                    ? 0 
                                                    : $indicador->idDimension;

        $dtaIndicador["strMetodologiaCalculo_ind"]  = ( is_null( $indicador->metodologia ) )
                                                        ? '' 
                                                        : $indicador->metodologia;
        
        $dtaIndicador["strLimitacionesTecnicas_ind"]    = ( is_null( $indicador->limitaciones ) )
                                                            ? '' 
                                                            : $indicador->limitaciones;
        
        $dtaIndicador["strInterpretacionIndicador_str"] = ( is_null( $indicador->interpretacion ) )
                                                            ? '' 
                                                            : $indicador->interpretacion;
        
        $dtaIndicador["strDisponibilidadIndicador_ind"] = ( is_null( $indicador->disponibilidad ) )
                                                            ? '' 
    ...
    public function updDtaGralIndicador( $indicador )
    {        
        $dtaIndicador["intCodigo_ind"]          = $indicador->idIndicador;
        $dtaIndicador["intCodigoTipo_ind"]      = $indicador->intCodigoTipo_ind;
        $dtaIndicador["inpCodigo_claseind"]     = $indicador->inpCodigo_claseind;
        $dtaIndicador["intCodigo_unimed"]       = $indicador->intCodigo_unimed;
        $dtaIndicador["inpCodigo_unianl"]       = $indicador->inpCodigo_unianl;
        $dtaIndicador["strNombre_ind"]          = $indicador->strNombre_ind;
        $dtaIndicador["strDescripcion_ind"]     = $indicador->strDescripcion_ind;
        $dtaIndicador["strFormula_ind"]         = $indicador->strFormula_ind;
        $dtaIndicador["inpCategoria_ind"]       = $indicador->inpCategoria_ind;

        $dtaIndicador["strMetodologiaCalculo_ind"]      = $indicador->strMetodologiaCalculo_ind;
        $dtaIndicador["strLimitacionesTecnicas_ind"]    = $indicador->strLimitacionesTecnicas_ind;
        $dtaIndicador["strInterpretacionIndicador_str"] = $indicador->strInterpretacionIndicador_str;
        $dtaIndicador["strDisponibilidadIndicador_ind"] = $indicador->strDisponibilidadIndicador_ind;
        $dtaIndicador["dteFechaModificacion_ind"]       = date("Y-m-d H:i:s");
     ...
     * 
     */
    public function getDtaSeguimientoIndicador( $idIndicador, $idIndVariable, $fchInicio = null, $fchFin = null )
    {
        try {
            $db = JFactory::getDBO();
            $query = $db->getQuery(true);
            ...
            if( !is_null( $fchInicio ) || !is_null( $fchFin ) ){
                $query->where( 't1.dteFecha_seg BETWEEN "'. $fchInicio .'" AND "'. $fchFin .'"' );
            }
            
            $query->order( 't1.dteFecha_seg' );
    ...
    /**
     * 
     * Retorna informacion de indicadores
     * 
     * @param type $idIndicador
     * @return type
     */
    public function getIndicador( $idIndicador, $fchInicio, $fchFin, $idEnfoque, $idDimension )
    {
        try {
            $db = JFactory::getDBO();
            $query = $db->getQuery( true );
            ...
            $query->select( '   DISTINCT 
                                    ...
                                    t1.idUnidadAnalisis,
                                    t1.unidadAnalisis,
                                    t1.idTpoUndMedida_UA,
                                    t1.tpoUndMedida_UA,
                                    t1.valorMeta,
                                    t1.idTipoUnidadMedida,
                                    t1.tipoUnidadMedida,
                                    t1.idUnidadMedida,
                                    t1.unidadMedida,
                                    t1.simboloUnidadMedida,
                                    t1.idHorizonte,
                                    t1.descripcionHorizonte,
                                    t1.fechaInicioIndicador,
                                    t1.fechaFinIndicador,

                                    CASE t1.idTendencia WHEN 1 THEN "Positiva" 
                                                        WHEN -1 THEN "Negativa"
                                    END AS tendencia,

                                    t1.idFrecuenciaMonitoreo,
                                    t1.frecuenciaMonitoreo,
                                    t1.idUndGestionResponsable,
                                    t1.idUndGestionFuncionario,
                                    t1.idFuncionarioResponsable,
                                    t1.idCategoriaIndicador,
                                    t1.categoriaIndicador,
                                    t1.fechaCreacion,
                                    t1.fechaModificacion' );
            $query->from( '#__dim_indicadores t1' );
            $query->join( 'LEFT' , '#__dim_dimension_enfoque t2 ON t1.idIndicador = t2.idIndicador' );

            //  Filtra informacion de un determinado indicador
            if( (int)$idIndicador != 0 ){
                $query->where( 't1.idIndicador = '. $idIndicador );
            }else{
                $query->where( 't1.fechaInicioIndicador = "'. $fchInicio .'"' );
                $query->where( 't1.fechaFinIndicador = "'. $fchFin .'"' );
            }
            
            //  Filtra informacion por un determinado indicador
            if( (int)$idEnfoque != 0 ){
                $query->where( 't2.idEnfoque = '. $idEnfoque );
            }
            
            //  Filtra informacion por un determinado indicador
            if( (int)$idDimension != 0 ){
                $query->where( 't2.idDimension IN ( '. $idDimension .' )' );
            }
     ...
     * @return type
     */
    public function getIndicadorPorEntidad( $idEntidad )
    {
        try {
            $db = JFactory::getDBO();
            $query = $db->getQuery( true );
            ...
            $query->from( '#__dim_indicadores t1' );
            $query->join( 'LEFT' , '#__dim_dimension_enfoque t2 ON t1.idIndicador = t2.idIndicador' );
            $query->where( 't1.idEntidad = '. $idEntidad );
    ...
    public function totalVE( $idIndEntidad )
    {
        try {
            $db = JFactory::getDBO();
            $query = $db->getQuery( true );

            $query->select( 'IF( SUM( t3.dcmValor_seg ) IS NULL, 0, SUM( t3.dcmValor_seg ) ) AS totalVE' );
            $query->from( '#__ind_indicador_entidad t1' );
            $query->join( 'INNER' , '#__ind_indVariable_indicador t2 ON t1.intCodigo_ind = t2.intCodigo_ind' );
            $query->join( 'INNER' , '#__ind_seguimiento t3 ON t2.intId_iv = t3.intId_iv' );
            $query->where( 't1.intIdIndEntidad = '. $idIndEntidad );
            $query->where( 't3.dteFecha_seg BETWEEN t1.dteHorizonteFchInicio_indEnt AND t1.dteHorizonteFchFin_indEnt' );
            
            $db->setQuery( (string)$query );
            $db->query();

            $te = ( $db->getNumRows() > 0 ) ? $db->loadObject()->totalVE 
                                            : false;
