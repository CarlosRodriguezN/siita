 ...
 */
class mantenimientoModelDpas extends JModelList {
    ...
    public function __construct($config = array()) {
        if (empty($config['filter_fields'])) {
            ...
            $config['filter_fields'] = array(   'inCodigo_ben',
                                                'intCodigo_grp',
                                                'strDescripcion_ben'
                                             );
        }
    ...
    public function getItems() {
        ...
        foreach ($items as &$item) {
            $item->url = JRoute::_('index.php?option=com_mantenimiento&amp;task=dpa.edit&amp;inpID_dpa=' . $item->inpID_dpa);
        }
    ...
    public function getListQuery() {
        ...
        $query->select('   dpa.inpID_dpa,
                           dpa.strDescripcion_dpa,
                           dpa.published,
                           u.name');
        $query->from('#__ut_dpa as dpa');

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin('#__users as u on u.id = dpa.checked_out');
        ...
        if ($published == '') {
            $query->where('( dpa.published = 1 OR dpa.published = 0 )');
        } else if ($published != '*') {
            $published = (int) $published;
            $query->where("dpa.published = '" . $published . "'");
        }
        ...
        if (!empty($search)) {
            ...
            $field_searches = " dpa.strDescripcion_dpa LIKE '" . $search . "'";
