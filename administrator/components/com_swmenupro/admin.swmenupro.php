...
switch (JRequest::getVar('task')) {
    ...
    case "exportMenu":
        ...
        header("location:index.php?task=showmodules&option=com_swmenupro&limit=$limit&limitstart=$limitstart");
        break;
...
//echo $absolute_path;
    ...
    $menus[] = JHTML::_('select.option', 'columnmenu', 'Sooperfish (Multi-Column beta)');
...
//echo $id."<br />";
    ...
    //echo "hello";
    header("location:index.php?option=com_swmenupro");
}
...
function removeMyMenu(&$cid, $option) {
    ...
    //setRedirect(JRoute::_("index2.php?task=showmodules&option=$option&limit=$limit&limitstart=$limitstart",$msg));
...
//echo $menustyle;
    if (!$id) {
        ...
        if ($menuid > 0) {
            ...
            if ($menustyle == "slideclick" || $menustyle == "clicktransmenu" || $menustyle == "accordian") {
                if (!$row2->slideclickmenu()) {
                    echo "<script> alert('" . $row->getError() . "'); window.history.go(-1); </script>\n";
                    exit();
                }
            }
            if ($menustyle == "gosumenu" || $menustyle == "transmenu" || $menustyle == "superfishmenu" || $menustyle == "columnmenu") {
                if (!$row2->gosumenu()) {
                    echo "<script> alert('" . $row->getError() . "'); window.history.go(-1); </script>\n";
                    exit();
                }
    ...
    if ($menustyle == "superfishmenu" || $menustyle == "gosumenu" || $menustyle == "transmenu") {
        ...
        $subindicator[] = JHTML::_('select.option', '5', 'blue_arrows');
    ...
    if ($menustyle == "transmenu" || $menustyle == "gosumenu") {
    ...
    echo "<script src=\"../media/system/js/core.js\" type=\"text/javascript\"></script> ";
    ...
    switch ($menustyle) {
        ...
        case "gosumenu":
            ...
            break;
        case "superfishmenu":
            require_once( JPATH_COMPONENT . DS . "admin.superfish.html.php");
            superfishMenuConfig($row2, $row, $menu, $pageNav, $padding1, $padding2, $border1, $border2, $border3, $border4, $modulename, $ordered, $parent_id, $orders2, $lists, $menuitems, $moduleclass_sfx);
            HTML_swmenupro::footer();
            break;
        case "slideclick":
        ...
        case "dynamictabmenu":
            ...
            break;
        case "columnmenu":
            require_once( JPATH_COMPONENT . DS . "admin.column.html.php");
            columnConfig($row2, $row, $menu, $pageNav, $padding1, $padding2, $border1, $border2, $border3, $border4, $modulename, $ordered, $parent_id, $orders2, $lists, $menuitems, $moduleclass_sfx);
            HTML_swmenupro::footer();
            break;
        case "accordian":
            require_once( JPATH_COMPONENT . DS . "admin.accordian.html.php");
            accordianConfig($row2, $row, $menu, $pageNav, $padding1, $padding2, $border1, $border2, $border3, $border4, $modulename, $ordered, $parent_id, $orders2, $lists, $menuitems, $moduleclass_sfx);
            HTML_swmenupro::footer();
            break;
    }
}
...
//$row->params="xvx";
    ...
    if (($count)) {
        ...
        $database->setQuery("UPDATE #__swmenu_config SET id = '" . $moduleID . "' WHERE id = '0'");
        $database->query();
    }
...
//	HTML_swmenupro::footer( );
        ...
        header("location:index.php?task=$returntask&option=com_swmenupro&limit=$limit&limitstart=$limitstart");
...
function exportMenu($id, $option) {
    ...
    if (count($swmenupro_array)) {
        ...
        $ordered = chain2('ID', 'PARENT', 'ORDER', $swmenupro_array, $parent_id, 25);
    }
    ...
    switch ($menustyle) {
        ...
        case "treemenu":
            ...
            break;
        case "gosumenu":
            $css = gosuMenuStyle($swmenupro, $ordered, $sub_indicator);
            break;
        case "transmenu":
        ...
        case "slideclick":
            ...
            break;
        case "superfishmenu":
            $css = superfishMenuStyle($swmenupro, $ordered, $sub_indicator);
            break;
        case "accordian":
...
function swGetMenuLinks2($menu, $id, $hybrid, $use_tables) {
    ...
    if ($menu == "swcontentmenu") {
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            $swmenupro_array[] = array("TITLE" => $result2->title, "URL" => $result2->link, "ID" => $result2->id, "PARENT" => $result2->parent_id, "ORDER" => $result2->lft, "IMAGE" => $result2->image, "IMAGEOVER" => $result2->image_over, "SHOWNAME" => $result2->show_name, "IMAGEALIGN" => $result2->image_align, "TARGETLEVEL" => $result2->target_level, "TARGET" => $result2->browserNav, "ACCESS" => $result2->access, "NCSS" => $result2->normal_css, "OCSS" => $result2->over_css, "SHOWITEM" => $result2->show_item);
...
function get_Version($directory) {
    ...
    if ($xml->loadFile($directory)) {
        $p = & $xml->document->getElementByPath('version');
    }
    $version = $p->_data;
...
function changeLanguage() {
    ...
    header("location:index.php?task=upgrade&option=com_swmenupro");
}
...
function upgrade($option, $installdir = "") {
    ...
    if ($com_entries != 1) {
        ...
        $database->setQuery("INSERT INTO `#__extensions` VALUES ( '', 'swmenupro', 'component', 'com_swmenupro', '', 0, 1, 0, 0, '{\"legacy\":true,\"name\":\"swMenuPro\",\"type\":\"component\",\"creationDate\":\"06\\/09\\/2010\",\"author\":\"Sean White\",\"copyright\":\"This Component is Proprietry Software\",\"authorEmail\":\"sean@swmenupro.com\",\"authorUrl\":\"http:\\/\\/www.swmenupro.com\",\"version\":\"7.2\",\"description\":\"Joomla 1.6 DHTML Menu Component\",\"group\":\"\"}', '{}', '', '', 0, '0000-00-00 00:00:00', 0, 0','1')");
        $database->query();
    }
...
function getPositions() {
    ...
    $query = 'SELECT DISTINCT(template) AS text, template AS value, client_id AS cid' .
