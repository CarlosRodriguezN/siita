 ...
 */
class PlanNacionalModelTiposIndicador extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array( 'descCategoria', 'descTipoIndicador', 'published' );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=tipoindicador.edit&amp;INTCODIGOTIPO_IND= '. $item->INTCODIGOTIPO_IND );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   ti.INTCODIGOTIPO_IND, 
                            c.STRDESCRIPCIONCATEGORIA as descCategoria,
                            ti.STRDESCRIPCIONTIPO_IND as descTipoIndicador,
                            ti.published,
                            u.name' );
        $query->from( '#__ind_tipo_indicador ti' );
        
        //  Cruzo informacion para obtener informacion de categoria
        $query->leftJoin( '#__gen_categoria as c on c.intCodigoCategoria = ti.intCodigoCategoria' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__users as u on u.id = ti.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( ti.published = 1 OR ti.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "ti.published = '{ $published }'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " c.STRDESCRIPCIONCATEGORIA LIKE '". $search ."' OR ".
