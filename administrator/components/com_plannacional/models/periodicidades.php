...
class PlanNacionalModelPeriodicidades extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array( 'descPeriodicidad', 'published' );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=periodicidad.edit&amp;codPeriodicidad= '. $item->codPeriodicidad );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   p.INTCODIGO_PER as codPeriodicidad, 
                            p.STRDESCRIPCION_PER as descPeriodicidad,
                            p.published,
                            u.name' );
        $query->from( '#__gen_periodicidad p' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__users as u on u.id = p.checked_out' );
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " p.STRDESCRIPCION_PER LIKE '". $search ."'";
