...
class PlanNacionalModelObjetivosNacionales extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array( 'planNacional', 'descripcion', 'published' );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   objn.INTCODIGO_ON, 
                            pn.STRDESCRIPCION_PN as planNacional,
                            objn.STRDESCRIPCION_ON as descripcion,
                            objn.published,
                            u.name' );
        $query->from( '#__pnd_objetivo_nacional objn' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__pnd_plannacional as pn on pn.INTCODIGO_PN = objn.INTCODIGO_PN' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__users as u on u.id = objn.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( objn.published = 1 OR objn.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "objn.published = '{ $published }'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " pn.STRDESCRIPCION_PN LIKE '". $search ."' OR ".
                              " objn.STRDESCRIPCION_ON LIKE '". $search ."' ";
