 ...
 */
class PlanNacionalModelIndicadoresNacionales extends JModelList
{
    public function __construct( $config = array() ) 
    {
        if( empty( $config['filter_fields'] ) ){
            ...
            $config['filter_fields'] = array( 'codIndNacional', 'descripcion', 'formula', 'planNacional', 'tipoIndicador', 'published' );
        }
    ...
    public function getItems()
    {
        ...
        foreach( $items as &$item ){
            $item->url = JRoute::_('index.php?option=com_plannacional&amp;task=indicadornacional.edit&amp;intCodigo_in= '. $item->intCodigo_in );
        }
    ...
    public function getListQuery()
    {
        ...
        $query->select( '   indnac.intCodigo_in as codIndNacional,
                            indnac.STRDESCRIPCION_IN as descripcion,
                            indnac.STRFORMULA_IN as formula,
                            pn.STRDESCRIPCION_MN as planNacional,
                            ti.STRDESCRIPCIONTIPO_IND as tipoIndicador,
                            indnac.published,
                            u.name' );
        $query->from( '#__pnd_indicador_nacional indnac' );

        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__pnd_meta_nacional as pn on pn.idcodigo_mn = indnac.idcodigo_mn' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__ind_tipo_indicador as ti on ti.INTCODIGOTIPO_IND = indnac.INTCODIGOTIPO_IND' );
        
        //  Verificamos si algun registro esta siendo editado por algun usuario
        $query->leftJoin( '#__users as u on u.id = indnac.checked_out' );
        ...
        if( $published == '' ){
            $query->where( '( indnac.published = 1 OR indnac.published = 0 )' );
        } else if( $published != '*' ){
            $published = (int)$published;
            $query->where( "indnac.published = '{ $published }'" );
        }
        ...
        if( !empty ( $search ) ){
            ...
            $field_searches = " indnac.STRDESCRIPCION_IN LIKE '". $search ."' OR ".
                              " indnac.STRFORMULA_IN LIKE '". $search ."' OR ".
                              " pn.STRDESCRIPCION_MN LIKE '". $search ."' OR ".
                              " ti.STRDESCRIPCIONTIPO_IND LIKE '". $search ."'";
