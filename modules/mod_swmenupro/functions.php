            ...
            while( $doSubMenu ){
                if( $ordered[$counter]['indent'] != 0 ){
                    ...
                    if( $sub_itemid == $ordered[$counter]['ID'] ){
                        $swid = 'id="click-sub-active' . $swmenupro['id'] . '"';
                    } else{
                        $swid = "";
                    }
...
function SlideClick( $ordered, $swmenupro, $active_menu, $expand, $parent_id )
{
        ...
        while( $doMenu ){
            ...
            if( $ordered[$counter]['indent'] == 0 ){
                ...
                while( $doSubMenu ){
                    if( $ordered[$counter]['indent'] != 0 ){
                        if( ($ordered[$counter]['indent'] == 1) && ($ordered[$counter - 1]['indent'] == 0) ){
                            $str.= '<div id="subsection' . $uniqueID . $topcounter . '" style="height:0px;font-size:0px" class="section' . $uniqueID . '">' . "\n";
                        }
                        $ordered[$counter]['URL'] = str_replace( '&', '&amp;', $ordered[$counter]['URL'] );
                        $name = swmenu_getname( $ordered[$counter] );
                        if( ($counter + 1 == $number) || ($ordered[$counter + 1]['indent'] == 0) ){
    ...
    for( $i = 0; $i < $topcounter; $i++ ){
        $str.= ($i < ($topcounter - 1)) ? "false, " : "false";
    }
    $str.=");\n";
        ...
        foreach( $topmenu as $top ){
            ...
            if( $do ){
                ...
                for( $i = 0; $i < count( $submenu ); $i++ ){
                    ...
                    if( $sub_itemid == $submenu[$i]['ID'] ){
                        $classname = 'inbox2-active';
                    } else{
                        $classname = 'inbox2';
                    }
                    if( $submenu[$i]['TARGETLEVEL'] == "0" ){
...
//echo $live_site.$mainframe->isAdmin();
    ...
    if( $active_menu && ($Itemid < 99998) && !$cookies ){
        $str.= "d" . $swmenupro['id'] . ".openTo(" . ($sub_itemid ? $sub_itemid : 0) . " , true);\n";
    }
    $str.="//-->\n";
    $str.="</script>\n";
...
function TigraMenu( $ordered, $swmenupro, $active_menu, $overlay_hack )
{
    ...
    while( $doMenu ){
        ...
        if( $ordered[$counter]['indent'] == 0 ){
            ...
            if( $ordered[$counter]['TARGETLEVEL'] == 1 || $ordered[$counter]['TARGETLEVEL'] == null ){
                $name = "'" . $name . "','" . $ordered[$counter]['URL'] . "',";
            } else{
                $name = "'" . $name . "','javascript:void(0);',";
            }
    ...
    if( $swmenupro['orientation'] == "vertical" ){
        ...
        $mymenu_content.= $swmenupro['main_width'];
    }
    $mymenu_content.=",\n";
    ...
    if( $swmenupro['orientation'] == "vertical" ){
        $mymenu_content.= $swmenupro['main_width'] . "px; height:" . (($swmenupro['main_height'] * $topcounter)) . "px \" >";
    } else{
        $mymenu_content.= (($swmenupro['main_width'] * $topcounter)) . "px; height:" . $swmenupro['main_height'] . "px \">";
    }
    $mymenu_content.= "\n<script type=\"text/javaScript\">\n";
    $mymenu_content.="<!--\n";
...
function DynamicTabMenu( $ordered, $swmenupro, $parent_id, $active_menu, $subwrap, $autoclose, $sub_active_menu )
{
    ...
    for( $i = 0; $i < count( $topmenu ); $i++ ){
        ...
        if( $top['TARGETLEVEL'] == 0 && $top['TARGETLEVEL'] != null ){
            $top['TARGET'] = 3;
        }
        switch( $top['TARGET'] ){
            // cases are slightly different
    ...
    foreach( $topmenu as $top ){
        ...
        if( $do ){
            ...
            foreach( @$submenu as $sub ){
                ...
                $name = swmenu_getname( $sub );
                if( $sub['TARGETLEVEL'] == 0 && $sub['TARGETLEVEL'] != null ){
                    $sub['TARGET'] = 3;
                }
                switch( $sub['TARGET'] ){
                    // cases are slightly different
